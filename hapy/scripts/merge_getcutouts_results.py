...
def find_result_files(indir, pattern="cutouts_summary*.ecsv"):
...
def main():
    ...
    parser.add_argument(
        "--indir",
        required=True,
        help="Root directory containing ecsv files."
    )
    ...
    parser.add_argument(
        "--pattern",
        default="cutouts_summary*.ecsv",
        help="Filename pattern to search for (default: cutouts_summary*.ecsv)"
    )
    parser.add_argument(
        "--out",
        default="merged_getcouts_results.fits",
        help="Output FITS filename (default: merged_getcutouts_results.fits)"
    )
