...
Test one galaxy:
```bash
run_analysis --cutout-dir
cutouts/VFID4168-NGC4237-CFHT-20160501-NGVS-4+3 --make-mask
--statmorph --galfit 
```

Just ran the test sample individually:
```
run_analysis --cutout-dir cutouts/VFID4212-MESSIER098-CFHT-20200524-NGVS-5+3 --make-mask --statmorph --galfit 
```

```
run_analysis --cutout-dir cutouts/VFID4496-NGC4193-CFHT-20180517-NGVS-5+1 --make-mask --statmorph --galfit
```
...
```bash
parallel --bar  -j 16  --memfree 30G --joblog run_analysis.joblog --results parallel-logs run_analysis --cutout-dir "{}" --make-mask --statmorph --galfit --no-gaia :::: cutouts_with_dir.txt
```
